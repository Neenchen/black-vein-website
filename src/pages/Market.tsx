...
function Prices({ title, items, currency = true }: { title: string; items: MarketItem[]; currency?: boolean }) { return <Col md={6}><article className="feature-card"><div className="eyebrow">Thistle market</div><h3 className="mt-2 mb-3">{title}</h3>{items.map((item) => <div className="market-row" key={item.name}><span className="d-flex align-items-center gap-2">{item.images?.map((image, index) => <img className="market-item-image" src={image} alt="" key={`${item.name}-${index}`} />)}<span><strong className="d-block fw-medium">{item.name}</strong>{item.note && <small className="text-secondary">{item.note}</small>}</span></span><span className="text-light text-nowrap">{item.price}{currency && <> <ThistleCurrency /></>}</span></div>)}</article></Col>; }
